...
class MorS {
  ...
  private MorS(MolGraph g) {
    if (g == null) try {
      ...
      System.out.println("MorS: Received null MolGraph: " + e.getMessage());
      e.printStackTrace();
      System.exit(-1);
    }
    this.smile = null;
  ...
  private MorS(String s) {
    if (s == null) try {
      ...
      System.out.println("MorS: Received null String: " + e.getMessage());
      e.printStackTrace();
      System.exit(-1);
    }
    this.smile = s;
...
public class MolSimilarity {
  ...
  public static Double similarity(Type type, MorS smile1, MorS smile2) {
    switch (type) {
      case DeltaC:
        return deltaCarbonsSimilarity(smile1, smile2);
      case CorrHeavyAtomsPearson:
        return corrHeavyAtomsLinearPearson(smile1, smile2);
      case CorrHeavyAtomsCount:
        return corrHeavyAtomsCount(smile1, smile2);
    }
  ...
  private static Double pearsonCorrelation(Integer[] X, Integer[] Y) {
    ...
    if (X.length != Y.length) {
      System.err.println("Correlation between unequal vectors?");
      System.exit(-1);
    }
    int len = X.length;
    ...
    if (stddevX == 0.0 || stddevY == 0.0) {
      System.err.println("Pearson only defined when stddev of X,Y finite and non-zero.");
      System.exit(-1);
    }
  ...
  private static Double deltaCarbonsSimilarity(MorS n1, MorS n2) {
    int c1, c2;
    if (n1.isSmile()) {
      c1 = countAtoms(n1.getSmile(), 'C');
      c2 = countAtoms(n2.getSmile(), 'C');
    } else {
      c1 = countAtoms(n1.getMolGraph(), new Atom(Element.C));
      c2 = countAtoms(n2.getMolGraph(), new Atom(Element.C));
    }
