...
/* This class computes the maximum weight perfect matching for a bipartite graph, i.e., the assignment problem
 * using the Hungarian Algorithm... 
 * The following are *not* applicable:
 * Hopcroft-Karp: maximum matching; we want weighted assignment
 * Edmonds-Karp / Ford-Fulkerson: maximum flow. does not work because very highly connected nodes might have multiple in/outs
 */
public class GraphMatching<N> {
	public static final String PartitionA = "NodesA"; // identifier for the metadata within the graph
	public static final String PartitionB = "NodesB"; // identifier for the metadata within the graph

	private List<Integer> partitionA, partitionB;
	private Graph<N,Double> graph;

	@SuppressWarnings("unchecked")
	public GraphMatching(Graph<N, Double> bipartite) {
		this.partitionA = (List<Integer>) bipartite.getMetaData(GraphMatching.PartitionA);
		this.partitionB = (List<Integer>) bipartite.getMetaData(GraphMatching.PartitionB);
		this.graph = bipartite;
	}

	private double[][] getWeightArrayFromBipartite() {
		int rows = this.partitionA.size();
		int cols = this.partitionB.size();
		
		double[][] weights = new double[rows][cols];
		
		for (int i = 0; i<rows; i++) {
			int rowNID = this.partitionA.get(i);
			for (int j = 0; j<cols; j++) {
				int colNID = this.partitionB.get(j);
				Double weight = this.graph.GetEdgeType(rowNID, colNID);
				if (weight == null)
					weight = 0.0; // this weight not exist; in which case we add a weight of 0? 
				weights[i][j] = weight;
			}
		}

		Logger.printf(10, "[Graph Matching] Weight matrix: [ ");
		for (int i = 0; i<rows; i++) {
			Logger.printf(10, "[");
			for (int j = 0; j<cols; j++)
				Logger.printf(10, weights[i][j] + " ");		
			Logger.printf(10, "] ");
		}
		Logger.printf(10, "]\n");
	...
	private List<P<Integer, Integer>> convertBackToBipartiteIndices(List<P<Integer, Integer>> matchIndices) {
		List<P<Integer, Integer>> matching = new ArrayList<P<Integer, Integer>>();
		List<Integer> AiMatched = new ArrayList<Integer>(), BiMatched = new ArrayList<Integer>();
		for (int i = 0; i<matchIndices.size(); i++) {
			int Ai = this.partitionA.get(matchIndices.get(i).fst());
			int Bi = this.partitionB.get(matchIndices.get(i).snd());
			AiMatched.add(Ai); BiMatched.add(Bi);
			matching.add(new P<Integer, Integer>(Ai, Bi));
		}
		// fill out the matching with -1 for those that remain unmatched....
		for (Integer Ai : this.partitionA)
			if (!AiMatched.contains(Ai))
				matching.add(new P<Integer, Integer>(Ai, -1));
		for (Integer Bi : this.partitionB)
			if (!BiMatched.contains(Bi))
				matching.add(new P<Integer, Integer>(-1, Bi));
		return matching;
	}

	public List<P<Integer, Integer>> getMaximumMatching() {
		Logger.printf(5, "[Graph Matching] Graph: %s\n[Graph Matching] Partition A: %s\n[Graph Matching] Partition B: %s\n", this.graph, this.partitionA, this.partitionB);
		
		String sumType = "max"; // max or min: we need the maximum weight matching: every elem is assigned to its most probable match
		double[][] array = getWeightArrayFromBipartite();
		
		boolean transposed = false;
		if (array.length > array[0].length)
		{
			Logger.printf(10, "[Graph Matching] Array transposed (because rows>columns).\n");	//Cols must be >= Rows.
		...
		for (int i=0; i<assignment.length; i++)
			if (!transposed)
				matching.add(new P<Integer, Integer>(assignment[i][0], assignment[i][1]));
			else
				matching.add(new P<Integer, Integer>(assignment[i][1], assignment[i][0]));
		Logger.printf(5, "[Graph Matching] The winning assignment (%s sum) is:%s\n", sumType, matching);
		return convertBackToBipartiteIndices(matching);
	}
}
