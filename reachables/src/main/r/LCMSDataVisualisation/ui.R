  ...
  navbarPage("Visualisation mode:",
             ...
             tabPanel("Multi",
               sidebarLayout(
                 sidebarPanel(
                   h3("Scans selection"),
                   textInput("filename1", label = "File name 1", value = "Plate_jaffna3_B1_0815201601.nc"),
                   textInput("filename2", label = "File name 2", value = "Plate_jaffna3_B1_0815201601.nc"),
                   textInput("filename2", label = "File name 3", value = "Plate_jaffna3_B1_0815201601.nc"),
                   sliderInput("retention.time.range", label = "Retention Time range",
                               min = 0, max = 450, value = c(130, 160), step = 5),
                   actionButton("load.multi", "Refresh scans!", icon("magic"), width = "100%", 
                                style="color: #fff; background-color: #337ab7; border-color: #2e6da4"),
                   h3("M/Z scope"),
                   selectInput("mode", label = "m/z mode", choices = kModes, selected = "M+H"),
                   numericInput("target.monoisotopic.mass", label = "Target monoisotopic mass", value = 463.184234, step = 0.001),
