...
object cascades {
  ...
  def write_node_cascades(p: String, depth: Integer, outputDirectory: String) {
    ...
    val reach: List[Long] = List(878L, 1209L, 552L, 716L, 475L, 4026L, 750L, 1536L, 1490L, 1496L, 341L, 448L, 1293L, 1443L, 45655, 19637L, 684L, 358L, 2124L, 6790L); println(s"Testing over ${reach.size} reachables. CONVERT TO ALL REACHABLES BEFORE MERGE!")
    ...
    reach.foreach(reachid => {
      val msg = f"id=$reachid%6d\tcount=${counter.getAndIncrement()}%5d\tCACHE: {cascades=${Cascade.cache_nw.stats.loadSuccessCount}%4d, pre_rxns=${Cascade.cache_bestpre_rxn.stats.loadSuccessCount}%4d, nodeMerger=${Cascade.nodeMerger.size}%5d}"
      Cascade.time(msg) {
