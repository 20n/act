...
object UntargetedMetabolomics {
  ...
  def main(args: Array[String]) {
    ...
    val inchis: PeakHits = if (!mapToInChIsUsingList) {
      ...
      PeakToStructure.StructureHits.toStructureHitsUsingLists(rslt, inchiListFile)
    }
    ...
    val formulae: PeakHits = if (!mapToFormulaUsingList) {
      ...
      PeakToStructure.FormulaHits.toFormulaHitsUsingLists(inchis, formulaListFile)
    }

    val formulaeWithSolver: PeakHits = if (!mapToFormulaUsingSolver) {
      formulae
    } else {
      PeakToStructure.FormulaHits.toFormulaHitsUsingSolver(formulae)
    }
